...
@Composable
fun ProviderAddScheduleScreen(
  reservationViewModel: ReservationViewModel
) {
  val addSchedule = reservationViewModel::updateProvider
  ...
  var timeStart by rememberSaveable { mutableStateOf(LocalTime.NOON) }
  var timeEnd by rememberSaveable { mutableStateOf(LocalTime.NOON.plusMinutes(15)) }
  ...
  fun submitSchedule() {
    ...
    val provider = Provider(
      providerId, listOf(TimeSlot(timeDateStart, timeDateEnd))
    )

    addSchedule(provider)
  ...
  Column(
    ...
    Text(stringResource(R.string.provider_id))
    TextField(
      value = providerId,
    ...
    val isSubmitEnabled = providerId.isNotEmpty() && isEndTimeValid
    Spacer(Modifier.height(16.dp))
    ElevatedButton(
